...
TARGETS = adam apple2 apple2enh atari c64 plus4 vic20 coco msdos pmd85 dragon
PROGRAM := fujinet.lib
...
all:
	@for target in $(TARGETS); do \
		...
		echo "-------------------------------------"; \
		$(MAKE) --no-print-directory -f makefiles/build.mk CURRENT_TARGET=$$target PROGRAM=$(PROGRAM) $(MAKECMDGOALS) || exit 1; \
	done
